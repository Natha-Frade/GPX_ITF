...
let _mounted = false
// entries: [{ name, data: File|Blob }]
//
// ATENÇÃO: no WORKERFS do emscripten, `files` espera objetos File DE VERDADE
// (ele lê file.name e file.size direto do item). Passando {name, data} ali,
// node.size vira undefined e o primeiro stat() dentro do wasm estoura
// "Cannot convert undefined to a BigInt". Para renomear o arquivo dentro do
// FS é preciso usar `blobs`, que é exatamente o formato {name, data}.
async function mountInputs(ff, entries) {
  for (const e of entries) {
    if (!(e.data instanceof Blob)) {
      throw new Error(`o arquivo "${e.name}" não está disponível localmente — recarregue-o no editor`)
    }
  }
  if (_mounted) {
  ...
  try { await ff.createDir('/inputs') } catch {}
  await ff.mount('WORKERFS', { blobs: entries }, '/inputs')
  _mounted = true
}

...
function _segLines() {
  ...
  // KMZ que só tem marcos de km (sem LineString) não gerava linha nenhuma
  // e o trecho saía reto. Aqui os marcos são encadeados em ordem de km e
  // viram uma linha de referência. É uma aproximação — corda reta entre
  // marcos, ~1 km cada — então serve para acompanhar o traçado geral,
  // não a curva fina. Para a curva fina, carregue o GPX do trecho.
  if (typeof kmzLayers !== 'undefined') {
    kmzLayers.forEach(l => {
      if (!l.visible) return;
      if (l.trackPolylines && l.trackPolylines.length) return;   // já tem linha real
      if (!l.kmPoints || l.kmPoints.length < 2) return;
      lines.push({
        name: l.name + ' (marcos de km)',
        aproximada: true,
        pts: l.kmPoints.map(p => ({ lat: p.lat, lng: p.lng })),
      });
    });
  }
  return lines;
...
function segHandleClick(lat, lng) {
  ...
  if (melhor) {
    ...
    snapOk = true;
    if (melhor.line.aproximada) road += ' — traçado aproximado';
  } else {
    // Cair na linha reta é quase sempre um dos dois casos abaixo. Antes
    // isso passava despercebido e o usuário só descobria olhando o mapa.
    path = [start, end0];
    if (!lines.length) {
      motivoReta = 'nenhuma via carregada';
    } else {
      let maisPerto = Infinity;
      lines.forEach(line => {
        const pa = _segProject(line, start.lat, start.lng);
        const pb = _segProject(line, end0.lat, end0.lng);
        if (pa && pb) maisPerto = Math.min(maisPerto, Math.max(pa.dist, pb.dist));
      });
      motivoReta = 'via carregada está a ' + Math.round(maisPerto) +
                   ' m (limite: ' + SEG_SNAP_MAX_M + ' m)';
    }
    showToast('Trecho em LINHA RETA — ' + motivoReta +
              '. Carregue o GPX ou o KMZ da via para o trecho seguir o traçado.',
              'error');
  }
  ...
  document.getElementById('segKmInfo').textContent =
    (kmIni && kmFim)
      ? `Km ${kmIni.estaca} → ${kmFim.estaca}  ·  ${extKm.toFixed(2).replace('.', ',')} km` +
        (snapOk ? `  ·  via: ${road}` : `  ·  ⚠ LINHA RETA (${motivoReta})`)
      : `${extKm.toFixed(2).replace('.', ',')} km` +
        (snapOk ? `  ·  via: ${road}` : `  ·  ⚠ LINHA RETA (${motivoReta})`) +

...
function exportPointsCSV() {
  ...
  // ── Por que este CSV é montado deste jeito ──────────────────────────
  //
  //  O separador aqui é ';' (padrão do Excel em português). Só que o
  //  Excel NÃO lê o arquivo: ele usa o "separador de listas" configurado
  //  no Windows daquela máquina. Num PC configurado em pt-BR isso é ';'
  //  e o arquivo abre certinho. Num PC em inglês (ou com a região
  //  trocada) isso vira ',' — e aí o Excel ignora os ';' e quebra o
  //  arquivo nas VÍRGULAS, inclusive nas que estão dentro das
  //  coordenadas e do valor de extensão. É por isso que a mesma planilha
  //  abre formatada numa máquina e toda embolada em outra.
  //
  //  Três medidas, juntas, resolvem em qualquer máquina:
  //
  //   1) A linha 'sep=;' no topo. O Excel lê essa linha ANTES de tudo e
  //      passa a usar ';' independentemente da configuração do Windows.
  //   2) Todo campo entre aspas, sem exceção.
  //   3) Coordenada não vai mais como "lat, lng" num campo só — vira
  //      quatro colunas de números. Some a vírgula de dentro do dado, e
  //      de quebra dá para usar a latitude e a longitude direto.
  const SEP = ';';
  const q = v => '"' + String(v == null ? '' : v).replace(/"/g, '""') + '"';
  const linha = campos => campos.map(q).join(SEP);

  const linhas = [
    linha(['Tipo', 'Marcação', 'Categoria', 'Km inicial', 'Km final',
           'Extensão (km)', 'Lat início', 'Lon início', 'Lat fim', 'Lon fim']),
  ];
  // Coordenada com PONTO decimal: é o formato que Google Earth, QGIS e
  // qualquer GPS esperam. A extensão vai com VÍRGULA, para o Excel em
  // português reconhecer como número e permitir somar.
  const gr = n => n.toFixed(6);

  segs.forEach(sg => {
    const cat = POINT_CATEGORIES.find(c => c.id === sg.category) || POINT_CATEGORIES[0];
    linhas.push(linha([
      'Trecho', sg.label, cat.label,
      sg.kmIni ? sg.kmIni.estaca : '',
      sg.kmFim ? sg.kmFim.estaca : '',
      sg.extKm.toFixed(2).replace('.', ','),
      gr(sg.start.lat), gr(sg.start.lng),
      gr(sg.end.lat), gr(sg.end.lng),
    ]));
  });
  savedPoints.forEach(pt => {
    const cat = POINT_CATEGORIES.find(c => c.id === pt.category) || POINT_CATEGORIES[0];
    linhas.push(linha([
      'Ponto', pt.label, cat.label,
      pt.km ? pt.km.estaca : '',
      '', '',
      gr(pt.lat), gr(pt.lng),
      '', '',

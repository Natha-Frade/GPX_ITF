...
// ──────────────────────────────────────────────────────────────────────
// Converte um texto de quilometragem em km decimal.
//  Entende: "322+560", "km 118+740", "KM-118+740", "Km 118 + 740",
//           "118,740", "118.740", "118".
//  Devolve null se não houver número.
//  IMPORTANTE: não dá pra usar parseFloat direto num texto de estaca —
//  parseFloat("118+740") devolve 118 e joga fora os 740 metros, que era
//  o motivo do km sair sempre como "118+000".
function parseKmValor(txt) {
  if (txt == null) return null;
  const s = String(txt).trim();

  // 1) Estaca NNN+NNN em qualquer posição (aceita prefixo e espaços)
  const est = s.match(/(\d+)\s*\+\s*(\d{1,3})\b/);
  if (est) {
    const km = parseInt(est[1], 10);
    const m  = parseInt(est[2], 10);
    if (!isNaN(km) && !isNaN(m)) return km + m / 1000;
  }

  // 2) Número solto (aceita vírgula decimal e prefixos tipo "KM-")
...
function extractKmLabel(pm, name, getEls) {
  ...
  for (const sd of allSD) {
    ...
    if (fields.some(f => fieldName.includes(f))) {
      ...
      if (val) {
        const num = parseKmValor(val);
        return { label: 'KM ' + val, km: num };
      }
    }
  }

  // FORMATO 2/3: o valor vem no <name> — pode ser estaca ("322+560",
  // "km 118+740") ou número puro ("118", "118,740").
  if (name) {
    const num = parseKmValor(name);
    if (num !== null && num > 0) {
      return { label: 'KM ' + fmtEstaca(num), km: num };
    }
  }

  // Fallback: usa o nome como está, sem valor numérico (fora da interpolação)

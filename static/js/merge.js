...
// ── Slots livres, na ordem A → B → C → D ─────────────────────────────
//  Usado por quem manda GPX de outras abas (CORTAR e VÍDEO). Antes esses
//  envios começavam sempre no A e sobrescreviam o que já estava lá; agora
//  eles ocupam o primeiro slot vago, então dá para ir acumulando cortes
//  de várias origens e só então juntar tudo.
function mergeSlotsLivres() {
  return SLOTS.filter(s => mergeData[s] === null);
}

// Quantos slots já estão ocupados.
function mergeSlotsOcupados() {
  return SLOTS.filter(s => mergeData[s] !== null);
}
...
function executeMerge() {
  ...
  // ── KM Inicial / Final em estaca (ex.: 313+261 → 298+992) ─────────
  //  Usa os marcos de km da via (KMZ carregado). Sem KMZ não há como
  //  saber a quilometragem, então mostra um traço.
  const pts0 = allSegments.flat();
  const kmIniEl = document.getElementById('mergeResKmIni');
  const kmFimEl = document.getElementById('mergeResKmFim');
  let kmIniTxt = '—', kmFimTxt = '—';
  if (pts0.length && typeof calcKmFromCoord === 'function') {
    try {
      const a = pts0[0], b = pts0[pts0.length - 1];
      const kA = calcKmFromCoord(a.lat, a.lng ?? a.lon);
      const kB = calcKmFromCoord(b.lat, b.lng ?? b.lon);
      if (kA && kA.estaca) kmIniTxt = kA.estaca;
      if (kB && kB.estaca) kmFimTxt = kB.estaca;
    } catch (_) { /* sem via carregada */ }
  }
  if (kmIniEl) kmIniEl.textContent = kmIniTxt;
  if (kmFimEl) kmFimEl.textContent = kmFimTxt;

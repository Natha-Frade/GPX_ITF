...
function sendCutsToMerge() {
  ...
  switchTab('unir');
  // Ocupa os slots VAGOS, na ordem A → B → C → D. Assim dá para mandar um
  // corte agora, outro depois, e eles vão se acumulando em vez de um
  // sobrescrever o outro — na hora de juntar, entram na ordem em que
  // foram enviados.
  ...
  const enviar = savedCuts.slice(0, livres.length);
  enviar.forEach((cut, i) => {
    const nome = 'corte_' + (cut.id + 1) + '.gpx';
    mergeInjectGpx(livres[i], nome, buildGpxString(cut.pts));
  });
  const sobra = savedCuts.length - enviar.length;
  showToast(`${enviar.length} corte(s) enviado(s) para ${enviar.length > 1 ? 'os slots' : 'o slot'} ` +
